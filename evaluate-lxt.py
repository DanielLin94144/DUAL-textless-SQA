...
from utils import AOS_score, Frame_F1_score, aggregate_dev_result, AOS_scores, Frame_F1_scores, calc_overlap
...
class SQAlxtDataset(Dataset):
    def __init__(self, data_dir):
        df = pd.read_csv(os.path.join(data_dir,'lxt_other_code.csv'))
        
        code_dir = os.path.join(data_dir, 'lxt_code')
        code_passage_dir = os.path.join(data_dir, 'lxt_code')
...
batch_size = 4
valid_dataset = SQAlxtDataset(data_dir)
...
output_df = pd.DataFrame(list(zip(df['hash'].values, start_secs, end_secs, pred_starts, pred_ends, f1s_after_sec, AOSs, dup)), 
                columns=['hash', 'gt_start', 'gt_end', 'pred_start', 'pred_end', 'f1', 'aos', 'dup'])
